...
const getUsersWithOffers = async (req, res) => {
  try {
    ...
    const totalUsersWithOffer = await UserModel.countDocuments(searchCriteria);

    const users = await UserModel.find(searchCriteria)
      .skip(skip)
      .limit(limit)
      .sort({ createdAt: -1 })
      .populate('offer') // Ensure offer details are populated
      .exec();

    console.log("Fetched Users with Offers:", JSON.stringify(users, null, 2)); // Log full data

    const totalPages = Math.ceil(totalUsersWithOffer / limit);
    ...
    console.error("Error in getUsersWithOffers:", err);
    res.status(500).json({
      success: false,
      message: "Internal server error",
      error: err.message,
...
module.exports = {
  createOrUpdateOffer, validateToken, useToken,
